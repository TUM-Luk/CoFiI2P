...
def test_acc(model,testdataloader,opt,topk_range = 5):
    ...
    for step,data in enumerate(testdataloader):
        ...
        sorted_dist, indices = torch.sort(dist_corr, dim=-1, descending=False) # 64个fine pixel与64个3d point的dist，升序排列
...
if __name__=='__main__':
    ...
    from data.carla import make_carla_dataloader, carla_pc_img_dataset
    ...
    args.dataset = "carla_0906_baseline"
    ...
    for epoch in range(1, opt.epoch):
        for step,data in enumerate(trainloader):
            ...
            writer.add_scalar("pc_score/inline_max", coarse_pc_inline_score.max(), global_step)
            writer.add_scalar("pc_score/inline_min", coarse_pc_inline_score.min(), global_step)
            writer.add_scalar("pc_score/inline_avg", torch.mean(coarse_pc_inline_score), global_step)
            writer.add_scalar("pc_score/outline_max", coarse_pc_outline_score.max(), global_step)
            writer.add_scalar("pc_score/outline_min", coarse_pc_outline_score.min(), global_step)
            writer.add_scalar("pc_score/outline_avg", torch.mean(coarse_pc_outline_score), global_step)
            ...
            if global_step % opt.val_freq==0: # 每个pc在自己GT对应的patch里，和16个pixel匹配的recall
                ...
                writer.add_scalar('fine_recall', fine_recall, global_step / 100.0)
            ...
            if global_step % opt.val_freq == 0:
                ...
                #                     'k=5': acc[4]}, 
                #                     int(global_step / 100))
                writer.add_scalar("test_acc/topk=1", acc[0], global_step)
                writer.add_scalar("test_acc/topk=2", acc[1], global_step)
                writer.add_scalar("test_acc/topk=3", acc[2], global_step)
                writer.add_scalar("test_acc/topk=4", acc[3], global_step)
                writer.add_scalar("test_acc/topk=5", acc[4], global_step)
